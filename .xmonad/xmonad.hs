...
tryPP h = defaultPP
	...
	} where i = "^i($HOME/.local/share/icons/bitmap/"
...
main = do
	bar <- spawnPipe panel
	info <- spawnPipe "./.config/dzen2/dzen2.sh"
	xmonad $ ewmh $ defaultConfig
		...
		, terminal = "urxvt"
		, borderWidth = 0
		, focusedBorderColor = "#F26190"
		, normalBorderColor = "#F692B2"
		, logHook = logbar bar
		} `additionalKeys` mKeys
		where panel = "dzen2 -fn 'M+ 1mn:style=Bold:size=10' -bg '#f9f9f9' -ta l -p -w 400 -y 10 -x 10 -h 24 -e ''"
